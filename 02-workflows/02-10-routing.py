...
def classify_query(state: SupportState) -> SupportState:
    ...
    return {"query_category": classification.category}
...
def handle_refund(state: SupportState) -> SupportState:
    ...
    return {"response": response, "tools_used": ["order_database", "refund_processor"]}
...
def handle_general(state: SupportState) -> SupportState:
    ...
    return {"response": response, "tools_used": ["knowledge_base", "faq_database"]}


def route_query(
    state: SupportState,
) -> Literal["billing", "technical", "refund", "general"]:
...
refund_prompt = (
    "I'd like to request a refund for my recent purchase. It doesn't meet my needs."
)
